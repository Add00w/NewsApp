...
struct ContentView: View {
    ...
    /// a transition that slides in from the leading edge when a view is inserted, and removes the view
    /// by scaling it down with fade
        ...
        Group {
            ...
            case .success:
                    ...
                    ForEach(filteredArticles) { article in
                        ArticleWidget(article:article).transition(transition)
                    }
                    .onDelete(perform:deleteArticle)
                    }
                    .animation(.default.speed(0.25), value: articles)
